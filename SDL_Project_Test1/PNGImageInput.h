...
const char* imageArray[KEY_PRESS_SURFACE_TOTAL] = { "SDL_Image_Imports/press.png", "SDL_Image_Imports/up.png",
            "SDL_Image_Imports/down.png", "SDL_Image_Imports/left.png", "SDL_Image_Imports/right.png" };
...
bool initWindow()
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (!(IMG_Init(imgFlags) & imgFlags)) // try ((IMG_Init(imgFlags) & imgFlags) != imgFlags)
            {
...
SDL_Surface* iloadSurfaceByArray(const char* path)
{
    ...
    SDL_Surface* loadedSurface = IMG_Load(path); //need to experiment here
